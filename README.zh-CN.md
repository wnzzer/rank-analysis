...
- **关系显示**：识别宿敌与好友。
- **导出 CSV / JSON**：当前筛选对局一键导出（基础字段或含补刀/经济/视野），支持复制到剪贴板。
- **聚焦详情模式**：展开对局即全屏查看，上一个/下一个 快速切换，Esc 收回。
...
- **官方 T 级榜单**：直连公开客户端 API，英雄/强化全量梯队展示与每日胜率排行。
- **英雄详情与流派出装**：推荐强化 TOP 组合、核心三件套、出门装与技能加点推荐。
- **选人期阵容启发式**：自动识别阵容缺口（前排 / AP / AD）与备选席（Bench）候选英雄打分排序。
- **三选一打分浮窗**：对局内智能识别强化卡牌，结合全服数据与当前英雄协同度实时打分。
- **对局内置顶悬浮窗（Overlay）**：透明置顶、支持鼠标穿透与 Alt+A 全局快捷键即时呼出。
...
- **习惯短板**：跨局聚合产出视野/补刀等六维短板标签，一键转为改错目标。
- **目标清单**：勾选追踪 + 本地备注 + JSON 备份/还原（支持拖拽导入）。
- **命令面板**：Ctrl K 全局唤起，最近使用自动前置。
- **战绩聚焦模式**：详情全屏查看，上一个/下一个 快速切换。
