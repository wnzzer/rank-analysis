 ...
 * （KDA/胜率/参团率/补刀速率/视野/经济/伤害），这里只负责任务与纪律框架。
 */
...
export function buildGrowthReportPrompt(recent: RecentData): string {
  ...
  sections.push(
    '===== 分析纪律（硬规则，必须遵守）=====',
    '- 【趋势事实】是确定性计算的结果：只能引用与解释，禁止改写数字、禁止自创数据。',
    '- 没有数据的维度（如分时曲线）一律不分析，不猜测。',
