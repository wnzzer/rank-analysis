...
#### 5.5.4 落地记录(2026-08-17)

上表 8 项差距逐项核对后的落地情况(commit `dbeff1f`/`31da61d`/`32bff18`,均已并入 fork/main):

| # | 维度 | 状态 | 落地说明 |
|---|---|---|---|
| 1 | 行卡信息架构 | ✅ 已完成 | 战绩详情展开层(10 人对比表/时间线/回放) + 行卡紧凑化 |
| 2 | 英雄视觉化 | ✅ 已完成 | 行卡头像 + 胜败描边 + MVP/金银行,详见 §4 方向 B 完成记录 |
| 3 | 技能/符文 | ✅ 已完成 | 方向 C:出装/符文推荐(C1/C3/C-2-UI/C-3-UI) |
| 4 | 响应式 | ✅ 已完成 | 窄窗左栏收抽屉 + 顶部 sticky 浮动分页(commit `7908582`) |
| 5 | 页头身份区 | ✅ 已完成 | PlayerBar 段位/胜点/胜率上提页头右侧 |
| 6 | 左栏丰富度 | ✅ 已完成 | UserSidePanel 左栏:RelationshipPanel 近期对手 + MettingPlayersCard |
| 7 | 交互便捷 | ✅ 已完成 | 回到顶部 FAB + 浮动分页 + 「收集更多/收集全部」跨区 SGP 无限拉取(commit `31da61d` 修复本区翻页 bug) |
| 8 | 数据形态 | ✅ 保持强化 | TrendBar 50 场趋势 + 四维筛选 + 英雄池联动,差异化卖点 |

**新增差异化能力(commit `32bff18`)**:玩家近期画像卡(`PlayerProfileCard`,hover 触发,挂战绩详情/对局玩家名——主玩位置/英雄池/本局熟练度/备注)与「玩家×英雄」双重加权阵容评分(`playerLineupAdjustment`:OP.GG 全局 meta 按玩家近期胜率/熟练度/补位平移,clamp 0.25-0.85)+ 双方对比条 `TeamStrengthBar`(选人期实时更新,数据不足整块隐藏,AI prompt 引用同一确定性分数)。同类工具(Akari/TGP/OP.GG)均无此双重加权。

**阵容评分质量优化(commit `51385b7`)**:三件事让分数可信、可解释、不卡顿:
1. **贝叶斯收缩**:小样本不再虚高——`shrunk = (wins+5)/(games+10)`(prior=0.5,强度 10),胜率调整 = `(shrunk-0.5)×0.3`,clamp [0.30,0.70]。5 场全胜只 +0.05 而非 +0.10,50 场则几乎贴原始胜率。
2. **breakdown 明细**:`LineupScore.breakdown` 逐英雄输出(全局胜率 → 调整后胜率 + 理由:近期胜率/绝活/首次使用/补位),TeamStrengthBar 常驻双列明细,调整过的英雄金色高亮、理由 hover 显示——分数从「黑盒数字」变成可解释的事实。
3. **选人期画像预取**:`useLineupScore` 新增 `prefetchProfiles`,进入选人页即预取 10 人画像(含未锁定),避开首次锁定后的请求峰值;失败静默,锁定后正常取数兜底。

#### 5.5.5 优化路线图全量落地记录(2026-08-17)

上节 P0 三件套之外,按路线图 P1/P2/P3/远期 逐项落地(commit `5027bbf`/`6aac3f7`/`59fcc95`/`a70a563`/`4c8a8a9`/`3ccd08d`/`ae33060`/`e610951`/`e8e01af`,均已并入 fork/main):

| 项 | 内容 | 落地说明 |
|---|---|---|
| P1-1 | SGP 画像源 | 跨区战绩兜底:LCU 本区空战绩且调用方给 region+name 时走 `getSgpMatchHistoryByName`(战绩按大区分存);SGP 失败降级空画像不阻塞;画像卡/详情页 context 透传 region |
| P1-2 | 按模式过滤 | 排位(420/440)≥5 场只用排位,不足回退全量(防画像真空);ARAM/竞技场场次不再稀释胜率 |
| P2-1 | 时间衰减权重 | 近 5 场权重 2x、更早 1x(状态趋势 > 长期均值),近期/英雄/本局英雄胜率三处同口径 |
| P2-2 | hover 阈值+翻转 | 竞态保护(请求序号,慢的旧请求不覆盖新玩家)+ NPopover 显式 flip(边界自动翻转) |
| P2-3 | 分位置英雄池 | 英雄池只统计本局位置场次(不看中单 100 场,看你打野场次),该位置 <2 场回退全位置 |
