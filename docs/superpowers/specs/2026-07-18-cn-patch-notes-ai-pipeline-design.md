...
- `fetch.mjs`：CMC 列表拉取与候选筛选。筛选比 `cn_patch_notes.rs` 现行规则更宽：
  标题含「更新公告」、排除「云顶」「手游」「周免」，站内文章（无跳转链）也进
  候选（正文经 CMC 内容接口取）。取最新一篇候选。
...
- 英雄名白名单：CI 内从 CommunityDragon 拉 zh_CN champion-summary，
  AI 输出的每个中文名必须精确命中，同时得到 `championId` 与英文 alias；
...
```jsonc
{
  ...
  "publishedAt": "2026-07-15",        // 来自列表 sIdxTime，新鲜度守卫用
...
- 不加 `[skip ci]`：push main 需要触发 sync-gitcode 镜像（quality-checks
  跟着跑一次无害，频率约两周一次）；
...
- **`cn_patch_notes.rs` 重写为瘦客户端**：删除 HTML 解析 / 方向判定 / CMC 逻辑
  ...
  → `https://raw.gitcode.com/wnzzer/rank-analysis/raw/main/data/patch-notes/cn-latest.json`）、
