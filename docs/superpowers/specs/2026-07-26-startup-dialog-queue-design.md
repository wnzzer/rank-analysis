...
| 交接留白 | 裁决后压住队列 300ms 再放行下一个。实测不压时 n-modal 的离场动画会与下一个的入场交叉,两张尺寸不同的卡片居中重叠约 200ms(旧卡 opacity 0.84 缩小 / 新卡 0.92 放大),观感是一次闪烁 |
...
- 全新配置(两个标记均缺失)启动:首屏就绪后先看到云同步告知,点「知道了」接着看到错误上报同意,**零帧共存**(前一张 opacity 归零后下一张才起步);
