...
| T6 ✅ | 协同搭档 UI：CounterHover 增「最佳搭档」节（synergies 数据已就绪）+ 单测 | `CounterHover.vue`、`+spec` | 单测绿 |
...
| v1.1 交付（终） | 2026-08-14 | T5/T6 完成：rustfmt 9 处 + clippy 5 处（编译×3 + lint×2）三轮迭代修复，CI 双矩阵全绿（`31795960055`）；T6 协同搭档节落地——`sortSynergies`/`formatSynergyLine`/`sortedSynergies` 纯函数 + CounterHover「最佳搭档」节（独立排序表头、副标题分隔、仅搭档有数据也渲染），新增 10 测试（service 5 + composable 2 + 组件 3）；前端全量 1049 单测绿 + 四件套门禁绿 + CI 全绿（`31797335422`）。全部任务卡 ✅，spec 关闭。 |
