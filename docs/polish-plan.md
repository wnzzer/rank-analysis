...
- [ ] R9-1 导出成功后的文件路径在 Growth/Record 以可复制文本短暂展示
      （message 换成带路径的可选中提示条，纯前端）
- [ ] R9-2 Home 快捷入口支持「恢复默认排序」动作
- [ ] R9-3 Growth 备份还原支持拖拽 .json 到页面即导入（dragover/drop，
      零依赖）
- [ ] R9-4 全局空态文案语气统一走查（去命令式、补下一步指引）
