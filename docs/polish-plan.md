...
## Round 13 —— 真机反馈驱动轮（候选池）

> 连续 12 轮迭代后，主干打磨已饱和。本轮起进入「反馈驱动」：
> 优先处理真实使用中发现的问题；无反馈时从下列候选取用。

- [ ] R13-a OverlayView 支持 maxItems 实时调整的视觉过渡（高度变化平滑）
- [ ] R13-b Record 聚焦模式在窗口宽度跨断点时的状态保持验证
- [ ] R13-c 导出 CSV 增加队伍列与对手英雄列（对位分析友好）
- [ ] R13-d Growth 备份文件支持多份轮换（按时间戳保留最近 5 份清单）
