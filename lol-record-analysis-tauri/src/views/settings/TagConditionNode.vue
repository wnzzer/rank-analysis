  ...
  <div class="condition-node">
    ...
    <div v-if="condition.type === 'and' || condition.type === 'or'" class="group-node">
      ...
      <div class="children-container">
        ...
        />
        <div v-if="(!condition.conditions || condition.conditions.length === 0)" class="empty-placeholder">
          (请添加条件)
        </div>
      </div>
    </div>
    ...
    <div v-else-if="condition.type === 'history'" class="leaf-node">
      <div class="node-header leaf-header">
        <span class="leaf-title">历史数据检查</span>
         <n-button size="tiny" circle type="error" text @click="removeSelf" v-if="!isRoot" class="remove-btn">
      ...
      <div class="leaf-body">
          ...
          <div class="filters-container">
            <div v-for="(filter, fIndex) in condition.filters" :key="fIndex" class="filter-row">
                <n-select size="small" :value="filter.type" @update:value="(val) => handleFilterTypeChange(filter, val)" :options="filterTypeOptions" class="sel-type" />
                ...
                <template v-if="filter.type === 'stat'">
                    <n-select size="small" v-model:value="filter.metric" :options="metricOptions" class="sel-metric" />
                    <n-select size="small" v-model:value="filter.op" :options="opOptions" class="sel-op" />
                    <n-input-number size="small" v-model:value="filter.value" :show-button="false" class="num-input" />
                </template>

                <n-button size="tiny" circle text type="error" @click="removeFilter(fIndex)" class="row-remove-btn">×</n-button>
            </div>
            <n-button size="tiny" dashed class="add-filter-btn" @click="addFilter">+ 添加筛选</n-button>
          </div>
          ...
          <div v-if="condition.refresh" class="refresh-row">
             <n-select size="small" v-model:value="condition.refresh.type" :options="refreshTypeOptions" class="sel-type" />
             
             <!-- Count/Sum/Avg/Max/Min -->
             <template v-if="['count', 'sum', 'average', 'max', 'min'].includes(condition.refresh.type)">
                <n-select 
                  ...
                  class="sel-metric"
                />
                <n-select size="small" v-model:value="condition.refresh.op" :options="opOptions" class="sel-op" />
                <n-input-number size="small" v-model:value="condition.refresh.value" :show-button="false" class="num-input" />
             </template>
             ...
             <template v-if="condition.refresh.type === 'streak'">
                <n-select 
                   ...
                   :options="[{label: '连胜', value: 'win'}, {label: '连败', value: 'loss'}]" 
                   class="sel-metric"
                />
                <span class="text-label">>=</span>
                <n-input-number size="small" v-model:value="condition.refresh.min" :show-button="false" class="num-input" />
    ...
    <div v-else-if="condition.type === 'currentQueue' || condition.type === 'currentChampion'" class="leaf-node">
         <div class="node-header leaf-header">
            <n-select size="small" v-model:value="condition.type" :options="[{label: '当前模式', value: 'currentQueue'}, {label: '当前英雄', value: 'currentChampion'}]" style="width: 120px" />
             <n-button size="tiny" circle text type="error" @click="removeSelf" v-if="!isRoot">✕</n-button>
         </div>
